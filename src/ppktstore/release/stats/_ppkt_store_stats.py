...
class PPKtStoreStats:
    """
    Calculate descriptive statistics about the phenopackets in the repository
    and generate figures to illustrate distributions.
    """
    ...
    def get_descriptive_stats(
        ...
        stats_d["genes associated with two diseases"] = len(
            [x for x in genes_to_disease_d.keys() if len(genes_to_disease_d[x]) == 2]
        )
        stats_d["genes associated with multiple diseases"] = len(
            [x for x in genes_to_disease_d.keys() if len(genes_to_disease_d[x]) > 2]
        )
        max_genes = set()
        max_count = 0
    ...
    def _get_disease_name(ppkt: Phenopacket) -> typing.Tuple[str, str]:
        if len(ppkt.diseases) == 1:
            ...
            raise ValueError(
                f"Cannot use this method for phenopackets with no or with multiple diseases (n={len(ppkt.diseases)})"
            )
